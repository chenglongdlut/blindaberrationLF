...
from blurkernel_general_GS import Blurkernel
...
def norm1(array_in):
    ...
    return (array_in-min_val)/(max_val-min_val)
## -------------------------------------------------------
def array2img(arr):

    arr_n=norm1(arr)

    img=np.uint8(arr_n*255)
    return img
## -------------------------------------------------------
class argument():
    def __init__(self):
        ...
        # self.data_path = "D:/Aberrations_study/deblur/VDIP-Deconvolution-main/VDIP-Deconvolution-main/datasets/LF_lytro"
        self.data_path ='./dataset/chart/'
        self.gt_path = ""
        ...
        self.learning_rate_kernel = 0.0002
...
def get_network():
    net = skip(opt.input_depth, opt.output_depth,
               ...
               num_channels_skip=[16, 16, 16,16,16,16],
...
for f in files_source:
    ...
    net_kernel=Blurkernel(opt.kernel_size[0],dtype)
    net_kernel = net_kernel.type(dtype)
        ...
        if step>2000:
            if (step + 1) % opt.save_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    cv2.imwrite(save_path, array2img(out_k_np))
                    ...
                    cv2.imwrite(save_path,array2img(out_x_np[padh // 2: out_x.shape[2] - padh // 2, padw // 2:  out_x.shape[3] - padw // 2, 0:3]))
