...
def get_crop_list(net_input_saved, patch_size, padh, padw):
    b, c, h, w = net_input_saved.shape

    # center_h = np.random.randint(patch_size // 2, h - padh - patch_size // 2)
    # center_w = np.random.randint(patch_size // 2, w - padw - patch_size // 2)
    crop_list = []

    # top left-> right bottom
    center_list = []
    step_size = patch_size // 2
    center_h = patch_size // 2
    while center_h < h - padh - patch_size // 2:
        center_w = patch_size // 2
        while center_w < w - padw - patch_size // 2:
            center_list.append([center_h, center_w])
            center_w += step_size
        center_list.append([center_h, center_w])
        center_h += step_size
    ...
    center_h = patch_size // 2
    while center_h < h - padh - patch_size // 2:
        center_w = w - padw - patch_size // 2
        while center_w > patch_size // 2:
            center_list.append([center_h, center_w])
            center_w -= step_size
        center_list.append([center_h, center_w])
        center_h += step_size
    ...
    center_list = []
    step_size = patch_size // 2
    center_h = h - padh - patch_size // 2
    while center_h > patch_size // 2:
        center_w = patch_size // 2
        while center_w < w - padw - patch_size // 2:
            center_list.append([center_h, center_w])
            center_w += step_size
